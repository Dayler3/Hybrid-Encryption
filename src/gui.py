...
class LoginWindow(ctk.CTkToplevel):
    ...
    def confirm(self):
        ...
        else: self.pwd.configure(fg_color="#7b241c")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.tabview.add("Шифрование"); self.tabview.add("Настройки")
        LoginWindow(self, self.conf.get("master_hash"), self.unlock)

    def unlock(self, pwd, new_hash=None):
        self.m_pwd = pwd
        if new_hash: self.conf["master_hash"] = new_hash; self.conf_manager.save(self.conf)
        if not os.path.exists("keys/private_key.pem"): self.km.generate_keys(pwd)
        self.setup_ui(); self.update_idletasks()
    ...
    def setup_ui(self):
        ...
        f_t = ctk.CTkFrame(tab, fg_color="transparent"); f_t.pack(fill="x", padx=50, pady=10)
        ...
        self.list = ctk.CTkScrollableFrame(tab, label_text="Очередь", border_width=2, corner_radius=10); self.list.pack(fill="both", expand=True, padx=20, pady=10)
        self.p_label = ctk.CTkLabel(tab, text="Очередь пуста"); self.p_label.pack()
        self.p_bar = ctk.CTkProgressBar(tab, width=450); self.p_bar.set(0); self.p_bar.pack(pady=5)
        self.btn_enc = ctk.CTkButton(tab, text="ЗАШИФРОВАТЬ", fg_color="#27ae60", height=40, command=lambda: self.start("encrypt")); self.btn_enc.pack(pady=5)
        self.btn_dec = ctk.CTkButton(tab, text="РАСШИФРОВАТЬ", fg_color="#2980b9", height=40, command=lambda: self.start("decrypt")); self.btn_dec.pack(pady=5)
        self.btn_stop = ctk.CTkButton(tab, text="ОТМЕНИТЬ", fg_color="#c0392b", height=40, command=self.proc.stop)
        f_b = ctk.CTkFrame(tab, fg_color="transparent"); f_b.pack(pady=10)
        ...
        self.log_box = ctk.CTkTextbox(self, height=120, font=("Consolas", 12)); self.log_box.pack(fill="x", padx=10, pady=10); self.log_box.configure(state="disabled")
        ctk.CTkLabel(tab, text="RSA-2048 | AES-256-CTR", font=("Roboto", 11), text_color="gray").pack(side="bottom", anchor="e", padx=20, pady=5)
        self.refresh(); self.setup_settings()

    def refresh(self):
        for w in self.list.winfo_children(): w.destroy()
        if not self.selected_files: self.p_label.configure(text="Пусто"); return
        sz = 0
        for f in self.selected_files:
            r = ctk.CTkFrame(self.list); r.pack(fill="x", padx=5, pady=2)
    ...
    def setup_settings(self):
        ...
        self.m_theme.set(self.conf.get("theme", "Темная")); self.m_theme.pack(pady=10)
    ...
    def add_row(self, tab, txt, path):
        ctk.CTkLabel(tab, text=txt).pack(); f = ctk.CTkFrame(tab); f.pack(fill="x", padx=30)
        e = ctk.CTkEntry(f); e.insert(0, path); e.pack(side="left", fill="x", expand=True, padx=5)
    ...
    def save_all(self):
        ...
        self.conf_manager.save(self.conf); self.write_log("Настройки сохранены.")

    def set_dir(self, e):
        d = filedialog.askdirectory()
        if d: e.delete(0, 'end'); e.insert(0, d)
    ...
    def remove_f(self, p):
        if p in self.selected_files: self.selected_files.remove(p); self.refresh()

    def clear(self): self.selected_files = []; self.refresh()

    def update_progress(self, val, eta):
        self.p_bar.set(val)
        if eta: self.p_label.configure(text=eta)

    def start(self, mode):
        if not self.selected_files: return
        self.btn_enc.pack_forget(); self.btn_dec.pack_forget()
        self.btn_stop.pack(pady=5, after=self.p_bar)
        self.write_log(f"Старт: {'Шифрование' if mode=='encrypt' else 'Расшифровка'}")
        self.proc.run_async(self.selected_files, mode, self.conf["encrypt_dir"] if mode=="encrypt" else self.conf["decrypt_dir"], self.update_progress, self.write_log, self.finish, self.m_pwd)
    ...
    def finish(self, s, t, time_taken, stopped):
        if not stopped:
            try: notification.notify(title="Hybrid Encryption v1.2", message=f"Готово: {s}/{t} за {time_taken}", timeout=10)
            except: pass
        self.btn_stop.pack_forget(); self.btn_enc.pack(pady=5, after=self.p_bar); self.btn_dec.pack(pady=5, after=self.btn_enc)
        self.write_log(f"ИТОГ: {s} из {t} готово за {time_taken}"); self.refresh(); self.p_bar.set(0)