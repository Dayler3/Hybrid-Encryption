...
class LoginWindow(ctk.CTkToplevel):
    def __init__(self, parent, saved_hash, on_success):
        ...
        ctk.CTkLabel(self, text="Мастер-пароль",
                     font=("Roboto", 16, "bold")).pack(pady=(30, 10))

        self.pwd_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.pwd_frame.pack(pady=10)
        self.pwd = ctk.CTkEntry(self.pwd_frame, show="*", width=180)
        self.pwd.pack(side="left", padx=(0, 5))
        self.pwd.bind("<Return>", lambda e: self.confirm())
        self.show_btn = ctk.CTkButton(
            self.pwd_frame, text="👁", width=35,
            fg_color="#34495e", command=self.toggle_pwd
        )
        self.show_btn.pack(side="left")
        ctk.CTkButton(self, text="ПОДТВЕРДИТЬ",
                      command=self.confirm, width=150).pack(pady=20)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.km = KeyManager()
        self.proc = CryptoProcessor(self.km)
        self.selected_files, self.m_pwd = [], None
        self.title("Hybrid Encryption v1.2")
        self.geometry("750x850")
        self.minsize(700, 800)
        theme = "Dark" if self.conf.get("theme") == "Темная" else "Light"
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(f_t, text="+ ФАЙЛЫ", command=self.add_f,
                      fg_color="#34495e").pack(side="left", expand=True, padx=5)
        ctk.CTkButton(f_t, text="ОЧИСТИТЬ", fg_color="#7f8c8d",
                      command=self.clear).pack(side="right", expand=True, padx=5)
        self.list = ctk.CTkScrollableFrame(tab, label_text="Очередь", border_width=2)
        ...
        self.btn_enc = ctk.CTkButton(
            tab, text="ЗАШИФРОВАТЬ", fg_color="#27ae60",
            height=40, command=lambda: self.start("encrypt")
        )
        self.btn_enc.pack(pady=5)
        self.btn_dec = ctk.CTkButton(
            tab, text="РАСШИФРОВАТЬ", fg_color="#2980b9",
            height=40, command=lambda: self.start("decrypt")
        )
        self.btn_dec.pack(pady=5)
        self.btn_stop = ctk.CTkButton(
            tab, text="ОТМЕНИТЬ", fg_color="#c0392b",
            height=40, command=self.proc.stop
        )
        f_b = ctk.CTkFrame(tab, fg_color="transparent")
        f_b.pack(pady=10)
        ctk.CTkButton(
            f_b, text="📁 Зашифрованные", width=140,
            command=lambda: FileSystem.open_explorer(self.conf["encrypt_dir"])
        ).pack(side="left", padx=5)
        ctk.CTkButton(
            f_b, text="📁 Расшифрованные", width=140,
            command=lambda: FileSystem.open_explorer(self.conf["decrypt_dir"])
        ).pack(side="right", padx=5)
        self.log_box = ctk.CTkTextbox(self, height=120, font=("Consolas", 12))
        self.log_box.pack(fill="x", padx=10, pady=10)
        self.log_box.configure(state="disabled")
        ctk.CTkLabel(
            tab, text="RSA-3072 | AES-256-GCM", font=("Roboto", 11),
            text_color="gray"
        ).pack(side="bottom", anchor="e", padx=20, pady=5)
        self.refresh()
        self.setup_settings()
    ...
    def refresh(self):
        ...
        for f in self.selected_files:
            ...
            ctk.CTkButton(
                r, text="✕", width=28, fg_color="#c0392b",
                command=lambda p=f: self.remove_f(p)
            ).pack(side="right", padx=5)
            try:
                size_sum += os.path.getsize(f)
            except OSError:
                pass
        txt = f"Файлов: {len(self.selected_files)} | {FileSystem.format_bytes(size_sum)}"
        self.p_label.configure(text=txt)

    def setup_settings(self):
        tab = self.tabview.tab("Настройки")
        self.m_theme = ctk.CTkOptionMenu(
            tab, values=["Темная", "Светлая"],
            command=lambda t: ctk.set_appearance_mode("Dark" if t == "Темная" else "Light")
        )
        ...
        ctk.CTkButton(tab, text="СОХРАНИТЬ", fg_color="#d35400",
                      command=self.save_all).pack(pady=20)
    ...
    def add_row(self, tab, txt, path):
        ...
        ctk.CTkButton(f, text="Обзор", width=60,
                      command=lambda: self.set_dir(e)).pack(side="right")
        return e

    def save_all(self):
        self.conf.update({
            "encrypt_dir": self.ent_enc.get(),
            "decrypt_dir": self.ent_dec.get(),
            "theme": self.m_theme.get()
        })
        self.conf_manager.save(self.conf)
    ...
    def start(self, mode):
        ...
        self.write_log(f"Старт: {'Шифрование' if is_enc else 'Расшифровка'}")
        out_dir = self.conf["encrypt_dir"] if is_enc else self.conf["decrypt_dir"]
        self.proc.run_async(
            self.selected_files, mode, out_dir,
            self.update_progress, self.write_log, self.finish, self.m_pwd
        )
    ...
    def finish(self, s, t, time_taken, stopped):
        if not stopped:
            try:
                notification.notify(
                    title="Hybrid Encryption v1.2",
                    message=f"Готово: {s}/{t} за {time_taken}",
                    timeout=10
                )
            except Exception:
                pass
